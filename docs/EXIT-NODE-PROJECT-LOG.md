...
|---|---|---|---|
| 1 | **`tailscale set --relay-server-port=41641`** on node 1 | node 1's in-memory prefs ONLY | ⚠️ **Wiped by ANY node-1 restart or recreate, not just `--reset` ones** — confirmed live 2026-08-25: after recreating node 1 with `--reset` already dropped (#77/#79), `RelayServerPort` was still gone, and `grep -c RelayServerPort /var/lib/tailscale/tailscaled.state` inside the container returned `0` both before and immediately after re-applying it. This setting is not written to `tailscaled.state` at all in this tailscaled version — it lives purely in the running daemon's memory. `--reset` was never the actual mechanism destroying it; #77/#79 only fixed the *original* stated problem (a stale `AdvertiseRoutes` resurrecting on `tailscale up --reset`). There is currently no declarative way to make this survive a restart — `RelayServerPort` isn't a `tailscale up` flag, only a `tailscale set` one, and nothing in this compose file's startup path re-asserts `tailscale set` flags on boot. **Nothing detects the loss** — no bats test, no healthcheck, no log line; this document is the only guard. Check: `ssh arr-stack-nas 'docker exec tailscale tailscale debug prefs'` → `RelayServerPort`. Re-apply after every node-1 restart/recreate: `ssh arr-stack-nas 'docker exec tailscale tailscale set --relay-server-port=41641'`. A real fix would need a startup hook (e.g. a healthcheck-adjacent script, or a `containerboot` post-up hook) that re-applies it automatically — not yet built, see §7 |
...
| #91 | `RelayServerPort` doesn't survive ANY node-1 restart | Discovered while deploying #77/#79 (2026-08-25) — dropping `--reset` did not fix this, it was never the cause. Needs a startup hook that re-applies `tailscale set --relay-server-port=41641` on every container boot (e.g. a `containerboot` post-up wrapper, or a healthcheck-adjacent script) — not yet built. Until then, manual re-apply after every node-1 restart, per §5 item 1 |
