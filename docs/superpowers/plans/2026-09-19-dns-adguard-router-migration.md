...
- [x] **8.2 Cold-boot test, before merging anything.** Reboot the NAS with the router answering, confirm gluetun reaches healthy and VPN egress works with no manual intervention. This is the assumption the 2026-08-27 38-hour outage falsified; do not assume it holds. Check the ingest timer and queue depth first — a reboot with an armed timer over a large watch folder is how the 2026-09-18 stall started, and it is not a DNS fault worth diagnosing inside this phase. See *Running this in parallel with other work*. — **Passed, 2026-09-21.** Pre-flight: `usenet-blackhole.timer` `disabled`/`inactive`, load 0.64, io PSI low, 12 files in staging. The branch was synced to the NAS (8.1 live, gluetun on `DNS_ADDRESS=192.168.110.1`) and the NAS rebooted.

  **Two minutes after boot, with no intervention:** gluetun `healthy`, VPN egress working (`79.127.134.20`), resolving both public and `.lan` names through the router; `sabnzbd`, `prowlarr`, `flaresolverr`, `magnetio-addon` all healthy and `vpn-socks5` running. `gluetun-recover` handled the boot case by itself — its log shows it restarting the stale-netns containers, which is the case it *can* handle and the manual recreate case it cannot (see 8.1). The NAS's own resolver came back as `192.168.8.1`, the router.

  **And the house never noticed.** A monitor polling the router every 2s from a VLAN20 client across the whole window: **99 samples, 99 resolved correctly, 0 failures** — including **36/36 during the 108 seconds the NAS was actually down**. `sonarr.lan` -> `192.168.110.250`, `doubleclick.net` -> `0.0.0.0` and a public name resolving, on every single sample. That is the migration's entire claim, measured through a real reboot rather than inferred.

  One measurement note, because the first reading of it was wrong: the monitor's own NAS-liveness column read `DOWN` on all 99 samples, including before the reboot. That is ICMP from VLAN20 to VLAN10 being filtered, not a NAS that was down — it was confirmed up over SSH immediately before the reboot. The DNS verdict is the column that means anything, and the NAS's real downtime was established separately by polling SSH until it answered.
...
- [x] **8.4 `traefik/dynamic/local-services.yml`** — the `pihole-lan` route. — **Retired, 2026-09-21.** It was parked until 9.4 on the assumption that Pi-hole would still be running in the meantime; it was not. 9.1 stopped Pi-hole, so both halves of this entry died then: `pihole.lan` reached a Traefik route whose upstream no longer answered, and the route answered with a basic-auth prompt and *then* failed, which reads like a credential problem rather than a retired service. Deferring it to 9.4 was wrong for a reason worth naming — **the step that broke it was the one before the step it was deferred to**, not the step itself.

  The routers and the service are gone; `pihole.lan` now 404s, which is honest, and `uptime.lan` still answers normally, so the rest of the file is intact. **Not repointed at AdGuard**, which is the obvious alternative and cannot work: AdGuard is on the router, its admin port is refused from every client VLAN by `zone_vlan10_input`, and Traefik is on the NAS on VLAN10 so it cannot reach it either. Proxying it would mean opening `router:3000` to VLAN10 — a segmentation decision, not a route change, and one nobody has made. The name itself goes with the service in 9.4.

  Incidental find: `pihole.png` 404s on the dashboard-icons CDN, so the old entry's icon had been broken already. `adguard-home.png` resolves.
...
- [x] **8.6 `scripts/lib/check-dns-duplicates.sh` and `scripts/lib/check-domains.sh`** — these read the NAS Pi-hole's config, and `check-dns-duplicates.sh` compares `02-local-dns.conf` against `pihole.toml` inside the container. Both stores are being retired, so the check goes with them; 3.5 is what replaces it. Do not retire them before 3.5 passes. — 3.5 passes, but **deferred to 9.4 rather than retired now**. Retiring them before the stores go would drop the only check that the NAS resolver is internally consistent, and Gate 5 deliberately keeps that resolver correct and revertible — so this is live coverage, not dead weight. One finding along the way: `check-dns-duplicates.sh` guards an unreadable `dnsmasq` side but not an unreadable `pihole.toml` side, so from 9.1 until 9.4 — while Pi-hole is stopped — it reports `OK: No duplicate DNS entries` without having looked at anything. The asymmetry is deliberate (`tests/lib-dns-duplicates.bats` states that an empty `pihole.toml` is the state the check wants) and the two cases are indistinguishable through the remote read, so it is recorded in the file rather than changed. **That window is the reason 9.4 must not slip.**
  — **Retired 2026-09-21, with the store.** Both checks and their four test/corpus files are gone; `scripts/pre-commit` lost checks 8 and 9 and the hook now runs nine. `tests/fixtures/dns-baseline.txt` survives: it is the 50-row oracle consumed by `scripts/dns-parity.sh`, `scripts/lib/dns-matrix.sh`, `scripts/lib/dns-parity.sh`, `tests/lib-dns-matrix.bats` and `tests/e2e/dns.spec.ts`, and has nothing to do with these two checks. `check-dns-divergence.sh` is now the only check comparing two stores at all. Two stale defaults went with them: `scripts/dns-matrix-check.sh` and `scripts/dns-parity.sh` both still defaulted to `192.168.110.246`, and a query sent there now meets the router's redirect and is answered by AdGuard — so the old default would have gone on "working" while naming a resolver that is not there. Both point at `192.168.8.1` and read `ROUTER_DNS_IP`, not `NAS_DNS_IP`.
- [x] **8.7 `homepage/config/services.yaml`** — the Pi-hole dashboard entry. — **Done, 2026-09-21**, for the same reason as 8.4: the widget polled `http://pihole:80`, which had been stopped, and the link went through the route 8.4 retired. Replaced with an **AdGuard Home** link and no widget — AdGuard's agent API is gated by GL.iNet's own auth middleware on this firmware (see the Phase 3 note), so a widget would show a permanent failure rather than a statistic, which is worse than no widget. The link points at `http://192.168.8.1:3000` and says so in its description, because it works from the maintenance VLAN and nowhere else.
...
- [x] **9.4 Only then** remove the two services from the compose file, back up the volumes first per the repo's backup convention, and land it through a PR. — **Done 2026-09-21**, before the week, and deliberately: a stopped container was not a safe state to wait in. Both carried `restart: always`, and Docker restarts a manually stopped `always` container when the daemon restarts, so a NAS reboot at any point in the week would have brought a resolver back that three VLANs used to depend on. `docker update --restart=no` (9.1) covered the reboot; removing the definitions covers everything else, including a future `docker compose up -d`.
  Backed up first, to `/volume1/docker/arr-stack-backups/`: `pihole-etc-pihole-20260921-200504.tgz` (29M, 37 entries) and `dnscrypt-config-20260921-200504.tgz` (75K, 14 entries). The Pi-hole one was **restored and read back** before removal — `pihole-FTL.db`, 70,017,024 bytes, header `SQLite format 3` — because an entry count is not a restore. Both volumes are still on the NAS; the containers are gone.
  Removed from `docker-compose.arr-stack.yml`: both service blocks, both volume declarations, and the `./pihole/dnsmasq.d` bind mount. **The directory stays** — `02-local-dns.conf.example` is the one file both DNS stores are generated from, and it holds every `.lan` name (18 records after 6 below retired `pihole.lan`). `configure_pihole()` went with them: its whole job was pointing the container at dnscrypt-proxy. `tests/no-nas-resolver.bats` is the guard, and it was written first and watched fail on the two definitions and the two `ipv4_address` lines.
- [x] **9.5 Record the retirement** in `docs/` with the measurements that justified it, in the style of the exit-node project log. — **Done 2026-09-21:** [docs/DNS-MIGRATION.md](../../DNS-MIGRATION.md), with the 99/99 reboot measurement, the commit-by-commit audit of #104-#115, nine corrections to this plan, the three guards that could not fail when first written, the measurement trap, and the live-only state. `docs/LOCAL-DNS.md`, `docs/ARCHITECTURE.md`, `docs/REFERENCE.md`, `docs/SETUP.md` and `CLAUDE.md` no longer describe a NAS resolver as live.
